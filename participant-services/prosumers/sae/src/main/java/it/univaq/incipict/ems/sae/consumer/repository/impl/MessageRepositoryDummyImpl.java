package it.univaq.incipict.ems.sae.consumer.repository.impl;
...
public class MessageRepositoryDummyImpl implements MessageRepository {
	...
	static {
		...
		logger.info("Initializing SAE repository");
	}
	...
	public <T> T get(String messageName, Class<T> t) {
		...
		logger.info("Getting from repo " + messageName + " with value " + messageJson);
	...
	public <T> void store(String messageName, T message) {
		...
		try {
			...
			logger.info("Storing into repo " + messageName + " with value " + messageJson);
